    ...
    std::unique_ptr<ASTNode> GenAstNode(std::string expressionLeft, std::string expressionFirst,
        ...
        if (expressionLeft == "programstruct") {
            if (expressionFirst == "program_head") {
                ...
                ASTNode *programstruct = new ProgramStruct(Unpack<ProgramHead>(subNodes[0]),
                                                           Unpack<ProgramBody>(subNodes[2]));
                return std::unique_ptr<ASTNode>(programstruct);
            }
        }
        if (expressionLeft == "program_head") {
            if (expressionFirst == "program") {
                //program_head -> program id program_head_89
                ASTNode *head = new ProgramHead(Unpack<OriASTNode>(subNodes[1])->content,
                                                Unpack<Identifiers>(subNodes[2]));
        ...
        if (expressionLeft == "idlist") {
            if (expressionFirst == "id") {
                ...
                idlist->Lines.insert(idlist->Lines.begin(), id->Line);
                idlist->Columns.insert(idlist->Columns.begin(), id->Column);
                return Pack(idlist);
            }
        }
        ...
        if (expressionLeft == "var_declaration") {
            if (expressionFirst == "idlist") {
                ...
                VariableDeclaration *variableDeclaration = new VariableDeclaration(
                        Unpack<TypeDeclarations>(subNodes[2]),
        ...
        if (expressionLeft == "var_declaration_81") {
            ...
            if (expressionFirst == ";") {
                ...
                VariableDeclaration *variableDeclaration = new VariableDeclaration(
                        Unpack<TypeDeclarations>(subNodes[3]),
            ...
            if (expressionFirst == "variable") {
                ...
                if (expression == nullptr) {
                    if (variable->varPart != nullptr && !variable->varPart->isProcedureCall)
                        logErrMsg(variable->Line, variable->Column,
                                  "Warning: there cannot be a single array without an assign expression");
                ...
                ASTNode *variableAssignStatement = new VariableAssignStatement(std::move(variable),
                                                                               std::move(expression));
        ...
        if (expressionLeft == "expression_list") {
            if (expressionFirst == "expression") {
                ...
                expressionList->expressions.insert(expressionList->expressions.begin(),
                                                   Unpack<Expression>(subNodes[0]));
        ...
        if (expressionLeft == "factor") {
            ...
            if (expressionFirst == "not") {
                ...
                ASTNode *notFactor = new NotFactor(
                        Unpack<Factor>(subNodes[1]), notSymbol->Line, notSymbol->Column);
